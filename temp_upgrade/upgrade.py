...
if os.name == 'nt':  # Windows
    UID = None  # Windows does not have a UID equivalent
else:
    UID = os.getuid()
DEFAULT_SSH_KEY_NAME = os.path.join(tempfile.gettempdir(), "%s_upgrade_ssh_key" % (UID or "default"))
KNOWN_HOSTS_NAME = os.path.join(tempfile.gettempdir(), "%s_upgrade_known_hosts" % (UID or "default"))
...
def upload_dump(dump_path, server, port, user, path, ssh_key, dest_dump_name=None):
    ...
    try:
        ...
        logging.error(
            "The upload failed. If you see a message like 'ssh: [..]: Connection refused/timed out' above,\n"
            ...
            "exception for the target host %s. Then, please re-run the script and when prompted, choose to resume.",
            server,
        )
        sys.exit(1)
