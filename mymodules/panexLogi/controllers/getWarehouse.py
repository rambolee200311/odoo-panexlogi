from odoo import http
from odoo.http import request


class GetWareHouseList(http.Controller):
    @http.route('/getWarehouseList', type='json', auth="user", cors="*", csrf=False)
    def getWarehouseList(self, **kw):
        # dingdan_h = 4900145711
        domain = []
        code=kw.get('code')
        if code:
            domain.append(('code', '=', code))
        warehouses = request.env['stock.warehouse'].sudo().search(domain)  # 随便找个模型查询一条数据

        if not warehouses:
            back_data = {'code': 300, 'msg': 'warehouse 不存在'}
            return (back_data)

        wearhouselist = []
        for r in warehouses:
            wearhouse = {
                "id": r.id,
                "code": r.code,
                "name": r.name,
            }
            wearhouselist.append(wearhouse)
        data = {
            "warehouses": wearhouselist
        }
        back_data = {'code': 100, 'msg': '查询warhouse成功', 'data': data}
        print("back_data==", back_data)
        return (back_data)


class GetLocationList(http.Controller):
    @http.route('/getLocationList', type='json', auth="user", cors="*", csrf=False)
    def getWarehouseList(self, **kw):
        # dingdan_h = 4900145711
        domain = []
        warehouse_id = kw.get('warehouse_id')
        if warehouse_id:
            domain.append(('warehouse_id', '=', warehouse_id))
        barcode=kw.get('barcode')
        if barcode:
            domain.append(('barcode', '=', barcode))

        locations = request.env['stock.location'].sudo().search(domain)  # 随便找个模型查询一条数据

        if not locations:
            back_data = {'code': 300, 'msg': 'location 不存在'}
            return (back_data)

        locationlist = []
        for r in locations:
            location = {
                "id": r.id,
                "barcode": r.barcode,
                "name": r.name,
                "warehouse_id": r.warehouse_id.id,
            }
            locationlist.append(location)
        data = {
            "locations": locationlist
        }
        back_data = {'code': 100, 'msg': '查询location成功', 'data': data}
        print("back_data==", back_data)
        return (back_data)
